...
while cap.isOpened():
    ...
    if frame_counter % FRAME_INTERVAL == 0:
        ...
        # print(road)
        for car in cars:
            car_id = car['id']  # Identyfikator samochodu (np. z YOLO lub inny unikalny)
                ...
                if car_id in cars_blocking_road:
                    ...
                    if abs(car_position[0] - previous_position[0]) < POSITION_TOLERANCE and \
                        ...
                        if elapsed_time >= BLOCK_TIME_THRESHOLD:
                            print(f"Samochód {car_id} blokuje drogę przez {elapsed_time:.2f} sekundy.")
                            print(f"Dokładniejsze dane o samochodzie: {car}")
                            # Możesz tutaj wywołać odpowiednią akcję (np. zapis do logów)
                    else:
